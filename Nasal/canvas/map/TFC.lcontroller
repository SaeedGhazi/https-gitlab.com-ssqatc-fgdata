...
SymbolLayer.add(name, {
	...
	df_options: { # default configuration options
		floor_ft:   9900, # Display target from this height below the aircraft
		ceiling_ft: 9900, # Display targets up to this height above the aircraft
...
var searchCmd_default = func {
	...
	foreach (var t; model_root.getChildren()) {
		...
		if (((tm.get_threat_lvl() or -1) > -1) and
			(min_alt < tm.get_alt()) and
